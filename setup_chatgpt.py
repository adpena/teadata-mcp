...
def main():
    ...
    if not config_dir.exists():
        ...
        print(
            "   Please ensure the ChatGPT macOS application is installed and has been run at least once."
        )
        sys.exit(1)
    ...
    if config_path.exists():
        try:
            ...
            print(
                "⚠️  Warning: Existing config.json was malformed. Starting with a fresh configuration."
            )
    ...
    if "teadata-mcp" not in python_path and ".venv" not in python_path:
        print(
            "⚠️  Warning: It looks like you might not be running this with the project's virtual environment."
        )
    ...
    print("ℹ️  Configuring 'teadata' MCP server...")
    ...
    server_config = {"command": python_path, "args": ["-m", "teadata_mcp"], "env": {}}
