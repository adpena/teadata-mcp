...
from teadata_mcp.sse_server import app, homepage
...
def test_sse_endpoint_connects():
    with patch("teadata_mcp.sse_server.transport", new_callable=MagicMock) as mock_transport:
...
def test_tool_endpoint_call():
    ...
    with TestClient(app) as client:
        ...
        with patch("teadata_mcp.sse_server.QueryRouter", return_value=mock_router):
            with patch("teadata_mcp.sse_server.DataEngineProvider"): # prevent real load
                with client: # Triggers startup
                    response = client.post(
                        "/api/tool/get_district",
